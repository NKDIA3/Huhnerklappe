 ...
 *  Vidit-systems
 *  15.01.19
 *  Präsentationsversion ohne Uhrzeit
 */
...
void loop() {
  ...
  if((State == 0 )&& (sensorValue >= 101)) {   // Wenn die Tür unten,hell,zwischen 5 und 18 Uhr --> Tür hochfahren
    hoch();
    delay(3800);
    aus();
    Serial.println("oben");
  ...
  if((State == 1 )&& (sensorValue <= 101)) {  // Wenn die Tür oben,dunkel,vor 5 und nach 18 Uhr --> Tür runterfahren    
    while( digitalRead(T1) == HIGH ){
      runter();
      delay(100);
      Serial.println("Fahre runter");
